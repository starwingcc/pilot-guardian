  ...
  return (
    <aside className="rule-rail" aria-label="访问规则">
      <div className="rail-heading">
        ...
        <Button size="icon" onClick={onAdd} aria-label="新建规则">
          <PlusIcon />
        </Button>
      </div>
      <InputGroup>
            ...
            filteredRules.map((rule, index) => (
              ...
              >
                ...
                <span className="rule-statuses">
                  {conflicts.has(rule.id) ? (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span className="conflict-icon" aria-label="与其他规则可能重叠">
                          <TriangleAlertIcon />
                        </span>
                      </TooltipTrigger>
                  ...
                  <Badge
                    variant={
                      dirtyRuleIds.has(rule.id) ? 'secondary' : rule.enabled ? 'default' : 'outline'
                    }
                  >
            ...
            <Empty>
              <EmptyHeader>
                <EmptyMedia variant="icon">
                  {rules.length === 0 ? <RadarIcon /> : <CircleOffIcon />}
                </EmptyMedia>
                <EmptyTitle>{rules.length === 0 ? '尚未部署航线' : '没有匹配规则'}</EmptyTitle>
                <EmptyDescription>
                  {rules.length === 0 ? '创建第一条访问规则。' : '尝试其他关键词。'}
                </EmptyDescription>
              </EmptyHeader>
              {rules.length === 0 ? (
                <EmptyContent>
